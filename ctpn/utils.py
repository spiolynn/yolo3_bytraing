...
def readxml(path):
    '''
    将xml信息 解析出gtboxes信息 和 filename 信息
    :param path:
    :return:
    '''
    gtboxes=[]
...
def cal_rpn(imgsize,featuresize,scale,gtboxes):
    '''
    计算rpn信息
    cal_rpn((h,w),(int(h/16),int(w/16)),16,gtbox)
    :param imgsize: 图像大小(h,w)
    :param featuresize: cnn(vgg) 大小
    :param scale:  缩放比例
    :param gtboxes: （n,4） box 坐标信息
    :return:
    '''
    imgh,imgw = imgsize
...
def gen_sample(xmlpath,imgpath,batchsize=1):
    ...
    while 1:
        ...
        if DEBUG:
            # 查看图片
            for (x1,y1,x2,y2) in gtbox:
                cv2.rectangle(img,(x1,y1),(x2,y2),(255,0,0),3)
            plt.subplot(111), plt.title('1'), plt.imshow(img,'gray')
            plt.show()
...
def testrpn():
    ...
    :return:
    '''
    imgpath = '../train_data/VOC2007/JPEGImages/img_1001.jpg'
    xmlpath = '../train_data/VOC2007/Annotations/img_1001.xml'
    ...
    anchors = anchors.astype(int)
    pos_anchors = base_anchor[cls==1]
    pos_anchors = pos_anchors.astype(int)
    neg_anchors = base_anchor[cls==0]
    neg_anchors = neg_anchors.astype(int)

    for i in neg_anchors:
        cv2.rectangle(img,(i[0],i[1]),(i[2],i[3]),(0,255,0),1)
...
if __name__ == '__main__':
    ...
    pass
    imgpath = r'C:\004_project\007-ocr\keras_ocr\data_process\TEXTVOC\VOC2007\\JPEGImages'
    xmlpath = r'C:\004_project\007-ocr\keras_ocr\data_process\TEXTVOC\VOC2007\\Annotations'
    # imgpath = '../train_data/VOC2007/JPEGImages'
    # xmlpath = '../train_data/VOC2007/Annotations'
