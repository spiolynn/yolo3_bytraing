...
def idokonw_v2(title_box,ctpn_boxes,label_boxes):

    # step3 去除IOU
    for i,title_box_i in enumerate(title_box):
        for j,ctpn_boxes_j in enumerate(ctpn_boxes):
            iou = bb_intersection_over_union(title_box_i,ctpn_boxes_j)
            if iou != 0:
                # 存在交集: 将ctpn坐标拉回正常
                ctpn_boxes[j][0] = title_box[i][2] + 1
